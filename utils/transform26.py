...
def pretrainedGhiasi():
  G = Ghiasi()
  import pathlib
  path = pathlib.Path(__file__).parent.resolve()
  
  G.load_state_dict(
    torch.load(f"{path}/weight/Ghiasi26.pth")
  )
  return G
...
class ConvRelu(nn.Module):
  def __init__(self,channels_in,channels_out,kernel_size,stride=1):
    ...
    # note : the batchnorm term is fusion to the conv weight
    # in magenta, they do not apply instance norm in first 3 layer
...
class UpsampleConvInRelu(nn.Module):
  def __init__(self, channels_in, channels_out, kernel_size, upsample, stride=1, activation=nn.ReLU):
    ...
    self.conv = nn.Conv2d(channels_in, channels_out, kernel_size, stride, bias=False)
